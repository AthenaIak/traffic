package trafficsimulation;

import java.awt.Color;
import java.util.Random;

/*==============================================================================
Implement Car class for NS model
===============================================================================*/

/*
 speed: 
 cells/t   m/s km/h
    1      7.5	27
    2		54
    3		81
    4		108
    5		135
    6		162
 */

public class CarNS {

    protected int ID;
    protected int speed;
    protected int lane;
    protected int position;
    protected int maxSpeed;
    protected Color color;
    protected int traveledDistance;

    public CarNS(int ID, int lane, int position) {
        this.ID = ID;
        this.lane = lane;
        this.position = position;
        this.traveledDistance = 0;
    }
    
    public void clearTraveledDistance() {
        traveledDistance = 0;
    }

    public int getTraveledDistance() {
        return traveledDistance;
    }

    public int getID() {
        return ID;
    }

    public int getSpeed() {
        return speed;
    }

    public void setLane(int lane) {
        this.lane = lane;
    }

    public int getLane() {
        return lane;
    }

    public void setPosition(int newPosition) {
        if (this.position + speed >= RoadNS.ROAD_SIZE) {
            traveledDistance += RoadNS.ROAD_SIZE - this.position;
            traveledDistance += newPosition;
        } else traveledDistance += newPosition - this.position;
        
        this.position = newPosition;
    }

    public int getPosition() {
        return position;
    }

    public Color getColor() {
        return color;
    }

    private String getType() {
        switch (this.getClass().toString()) {
            case "class trafficsimulation.SlowCarNS":
                return "S";
            case "class trafficsimulation.FastCarNS":
                return "F";
        }
        return "E";
    }

    @Override
    public String toString() {
        return "(" + getType() + " " + lane + "," + position + "," + speed + ") ";
    }

    /*
     * This method adjusts the speed of the car based on neighbour cars
     */
    public int adaptSpeed(SpeedDistance carFront, SpeedDistance carFrontNextLane, SpeedDistance carBehindNextLane){
            
//        if (RoadNS.DEBUG)
//            System.out.print("Lane " + lane + " pos " + position + " speed " + speed + 
//                    ", sFront " + carFront.getSpeed() + ", dFront " + carFront.getDistance() + 
//                    ", sFrontNext " + carFrontNextLane.getSpeed() + ", dFrontNext " + carFrontNextLane.getDistance() + 
//                    ", sBehindNext " + carBehindNextLane.getSpeed() + " dBehindNext " + carBehindNextLane.getDistance() + ". ");

        boolean goodGapToChange = (carBehindNextLane.getDistance() >= RoadNS.MAX_SPEED_FAST_CAR && carFrontNextLane.getDistance() >= speed);
        
        if (carFront.getDistance() <= RoadNS.DISTANCE_TO_LOOK_AHEAD || carFrontNextLane.getDistance() <= RoadNS.DISTANCE_TO_LOOK_AHEAD){            
            
            if (lane==RoadNS.RIGHT_LANE) {
                if ((carFront.getSpeed() <= speed || carFrontNextLane.getSpeed() <= speed) && goodGapToChange) {
                    lane = RoadNS.LEFT_LANE;     // switch lane
                    if (RoadNS.DEBUG)
                        System.out.print("Change to left lane. ");  
                }
            } else {
                if ((carFront.getSpeed() > speed && carFrontNextLane.getSpeed() > speed) && goodGapToChange) {
                    lane = RoadNS.RIGHT_LANE;     // switch lane
                    if (RoadNS.DEBUG)
                        System.out.print("Change to right lane. ");                      
                }
            }            
        }
        
        // adjust speed        
        if (speed < maxSpeed) speed += 1;
        if (speed > carFront.getDistance()) speed = carFront.getDistance();
        if (speed >= 1){
            Random r = new Random();
            if (r.nextDouble() < RoadNS.PROBABILITY_FLUCTUATION) speed--;
        }
        
        return speed;
    }
    
}
