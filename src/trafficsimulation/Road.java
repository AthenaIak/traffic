...
public class Road {
    ...
    private void moveCar(Car car) {
        ...
        int nextCar = getPositionOfNextCar(position, lane);
        int distanceToNextCar = Math.floorMod(nextCar - position - 1, TrafficSimulation.ROAD_SIZE);
        int nextCarSpeed;
        if (lane == 1) {
            nextCarSpeed = l1[nextCar];
        } else {
            nextCarSpeed = l2[nextCar];
        }
    ...
    private int getPositionOfNextCar(int current_pos, int lane) {
        ...
        for (int i = current_pos + 1; i < TrafficSimulation.ROAD_SIZE + current_pos; i++) {
            ...
            if (lane == 1 && l1[correct_i] != 0) {
                return correct_i;
            } else if (lane == 2 && l2[correct_i] != 0) {
                return correct_i;
            }
        }
        return -1;
